...
function shapedAppStateSql(runId, c, appC, pendingBills, targetBytes, updatedBy) {
  ...
  const sessionItems = `coalesce((select jsonb_agg(${marked(`'id',${id(runId,"session-item","sg")},'inventoryItemId','qa-scale','name','QA Performance Item','quantity',0,'unitPrice',0,'addedAt',${recentTime("sg")}`)} order by sg) from generate_series(g,${represented(c.session_items)},greatest(1,${appC.sessions})) sg),'[]'::jsonb)`;
  const tabItems = `coalesce((select jsonb_agg(${marked(`'id',${id(runId,"tab-item","tg")},'inventoryItemId','qa-scale','name','QA Performance Item','quantity',0,'unitPrice',0,'addedAt',${recentTime("tg")}`)} order by tg) from generate_series(g,${represented(c.customer_tab_items)},greatest(1,${appC.customerTabs})) tg),'[]'::jsonb)`;
...
function seedStatements(runId, snapshot, plan, production, packageBindingSha256) {
  ...
  inserts.push(insertSql(runId,c.combo_fixed_items,"combo_fixed_items",["organization_id","combo_id","id","sellable_option_id","quantity","raw_data","created_at","updated_at"],[q(ORGANIZATION_ID),comboAnchor,id(runId,"combo-fixed"),id(runId,"sellable-option"),"0",marker(runId),oldTime(),oldTime()]));
  ...
  inserts.push(insertSql(runId,c.session_items,"session_items",["organization_id","session_id","id","inventory_item_id","name","quantity","unit_price","added_at","raw_data","created_at","updated_at"],[q(ORGANIZATION_ID),sessionAnchor,id(runId,"session-item"),"null",q("QA Performance Item"),"0","0",oldTime(),marker(runId),oldTime(),oldTime()]));
  ...
  inserts.push(insertSql(runId,c.customer_tab_items,"customer_tab_items",["organization_id","customer_tab_id","id","inventory_item_id","name","quantity","unit_price","added_at","raw_data","created_at","updated_at"],[q(ORGANIZATION_ID),tabAnchor,id(runId,"tab-item"),"null",q("QA Performance Item"),"0","0",oldTime(),marker(runId),oldTime(),oldTime()]));
