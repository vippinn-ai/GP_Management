...
const allowedModes = new Set(["--verify", "--remaining-two", "--verify-remaining-two"]);

if (args.length > 1 || args.some((argument) => !allowedModes.has(argument))) {
  throw new Error("Checkout-session-item race preflight accepts only --verify, --remaining-two, --verify-remaining-two, or one exact preflight.");
}
const mode = args[0] ?? "--all";
const verificationOnly = mode === "--verify" || mode === "--verify-remaining-two";
const scenarios = mode === "--remaining-two" || mode === "--verify-remaining-two"
...
const stationName = env.E2E_SESSION_ITEM_RACE_STATION?.trim() || "8 Ball Pool";
...
if (new URL(supabaseUrl).hostname !== `${STAGING_PROJECT_REF}.supabase.co`) {
  throw new Error("Checkout-session-item race preflight is locked to staging.");
}
...
// Keep username resolution sequential; the staging resolver is intentionally not raced.
...
if (temporaryAdmin && observer.identity.actorId !== temporaryAdmin.actorId) {
  throw new Error("Slot B did not authenticate as the exact temporary staging admin bound by its credential checkpoint.");
}
const supabase = origin.client;
const [artifact, openSessions, openTabs, appState, stations, categories, items, sessions, bills] = await Promise.all([
  deployedArtifact(),
  ...
  supabase.from("stations").select("id,name,mode,active")
    .eq("organization_id", organizationId).eq("name", stationName),
  ...
  supabase.from("sessions").select("id,customer_name,status")
...
for (const [label, result] of Object.entries({ openSessions, openTabs, appState, stations, categories, items, sessions, bills })) {
...
const artifactPath = path.join(artifactDirectory, `checkout-session-item-race-preflight-${runId}.json`);
...
const evidence = {
  ...
  identityCollisions: { items: items.data, sessions: sessions.data, bills: bills.data, artifacts: artifactCollisions },
  appState: {
...
if (verificationOnly) {
  if (!fs.existsSync(artifactPath)) throw new Error("The reviewed checkout-session-item preflight artifact is missing.");
  const reviewed = JSON.parse(fs.readFileSync(artifactPath, "utf8"));
  const matches = reviewed.safeToRun && evidence.safeToRun &&
    reviewed.runId === runId &&
    ...
    reviewed.actorsDistinct === true && evidence.actorsDistinct === true &&
