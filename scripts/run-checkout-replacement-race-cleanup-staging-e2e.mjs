...
if (args.length > 1 || args.some((argument) => argument !== "--list")) {
  throw new Error("Replacement-race cleanup accepts only --list or one exact execution.");
}
...
if (!discoveryOnly) {
  ...
  if (!requestedCleanupRunId) throw new Error("An explicit E2E_RUN_ID is required for replacement-race cleanup.");
  const cleanupRunId = sanitizeRunId(requestedCleanupRunId);
  const requested = childEnv.E2E_REPLACEMENT_RACE_RECOVERY_ARTIFACT?.trim();
  if (!requested) throw new Error("E2E_REPLACEMENT_RACE_RECOVERY_ARTIFACT is required.");
  const recoveryPath = path.resolve(root, requested);
  const recoveryDirectory = path.resolve(root, "test-artifacts", "reconciliation");
  if (path.dirname(recoveryPath) !== recoveryDirectory || !/^checkout-replacement-race-recovery-[A-Za-z0-9_-]+\.json$/.test(path.basename(recoveryPath))) {
    throw new Error("Cleanup accepts only an immutable replacement-race recovery artifact.");
  }
  const recovery = JSON.parse(fs.readFileSync(recoveryPath, "utf8"));
  if (recovery.safeForIdentityBoundCleanup !== true || recovery.safeForAutomaticRetry !== false || recovery.productionAllowed !== false) {
  ...
  childEnv.E2E_REPLACEMENT_RACE_FIXTURE_RUN_ID = fixtureRunId;
  childEnv.E2E_REPLACEMENT_RACE_RECOVERY_ARTIFACT = path.relative(root, recoveryPath);
}
