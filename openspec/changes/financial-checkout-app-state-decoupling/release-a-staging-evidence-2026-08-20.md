...
- Execution window: `2026-08-19T19:28:17Z` through at least `2026-08-20T12:23:37.599Z` (`2026-08-20 00:58` through at least `17:53:37` IST) for the recorded active runs; the required business-day soak remains open unless explicitly waived as an expedited-release risk decision.
...
## Immediate consolidated zero-retry acceptance

At the user's request to replace an idle elapsed-time wait with immediate scripted evidence, guarded Playwright run `20260820122218` executed all five Release A staging cases in one worker with zero retries. All five passed in `77.044 s`: two-browser pause edit and hard refresh, two-browser hop/detach plus exact-session v1 billing, inventory fixture discovery, the combo/variant/cigarette reservation and cleanup matrix, and complete normalized report CSV/XLSX/PDF exports. No page error, failed cleanup, or production reference was reported.

The first immediate run, `20260820121318`, retained a harness failure rather than relabeling it. The origin committed exactly one `edit_pause_log`, but the observer test entered edit mode before the compact event arrived and froze the old value in a local draft. Read-only database verification proved pause `pause-83a4c871-84cd-4506-aff4-f5991de355a9` was correctly stored as `2026-08-20T12:11:00+00:00` through `2026-08-20T12:12:00+00:00`; event `event-5c4edc18-5e79-4e1f-8a72-f5f8a0ce6cbb` carried the same pause ID and mutation `op-6e523fdb-88f3-463b-8e58-40d86162ae1e`. Cleanup rejected the exact QA session once. The corrected contract waits for the observer's rendered read-only timestamp before opening its edit draft; focused run `20260820121812` and the consolidated run both passed without refresh.

The separately skipped hop case was then run as `20260820121851`. Its functional chain completed exactly once, including detach audit and v1 bill `BILL-20260820-007`; only the obsolete assertion failed because `Change Customer` correctly closed the continuation dialog before the test searched inside it. The harness now asserts dialog closure, origin detach audit, and observer detach audit. Focused run `20260820122036` passed, followed by the consolidated pass. In consolidated run `20260820122218`, session `session-de235c18-2673-47bb-b495-019d52f9f4d1` was hopped once, detached once, and billed exactly once as `BILL-20260820-009` / `bill-33446023-89e1-4692-8686-73f8e3397f57`; the checkout response's changed session ID matched the source session.

The same consolidated run reserved Maggie/Malboro Compact/Thumsuyp exactly `2/10/1` from baseline availability `18/96/87`, showed those reservations in both browser contexts and the normalized Inventory Report, rejected only tab `customer-tab-1d241dd7-ebce-4ab7-b8c7-6b21113e3c83`, and restored exact availability after independent hard refresh. Report exports produced a 1,101-byte CSV, an 18,813-byte XLSX with nine rows, and a 5,002-byte PDF. A read-only staging query also found seven pre-existing unbilled hopped sessions dated April through July; five have named non-QA customers and two are old `QA Hop 20260727 A` rows. They predate this run and were not modified.

This immediate pass can replace the functional portion of a passive staging wait, because no ordinary staff traffic exists in staging. It does not create elapsed operational evidence. Under the approved runbook, Release A promotion therefore still requires either the documented soak or an explicit risk acceptance waiving that gate. More importantly, Release A retains `commit_checkout_bill` v1 and its compatibility `app_state` rewrite; it is a prerequisite read-path release and does not itself remove the production checkout-latency cause. Financial v2 remains uninstalled and disabled on staging.

## Current gate decision

Database reconstruction, additive Release A installation, staging frontend deployment, authenticated normalized-read smoke, representative timed/unit/tab v1 checkout, settlement, replacement, write-off, refund, customer-history correction, actor/stock verification, independent Chrome session/item/reject and financial realtime coverage, the complete customer-tab item add/update/remove realtime matrix with exactly-once quantity mutation, no-refresh pause deletion overlay, two-browser pause edit, two-browser hop/detach with exact-session billing reconciliation, Bill Register invalidation correction, receipt and report CSV/XLSX/PDF exports, historical receipt linkage, analytics, customer-history, the combo/variant/cigarette reservation and refresh matrix, hard-refresh non-resurrection checks, permission persistence/restoration, the isolated live fail-closed exercise, the Gate 6 database/error/performance capture, and a final consolidated five-case zero-retry Playwright run are complete. Release A is functionally accepted on staging but remains a runbook no-go until the business-day soak is completed or explicitly waived and the independent reviewer signs off on that decision. Financial v2 remains a separate uncompleted Release B gate, so this evidence does not authorize claiming that production checkout latency is fixed. No production promotion claim is made.

## Remaining gated work

1. Complete the full representative staging business-day soak, or explicitly accept the risk of waiving elapsed soak because staging has no ordinary staff traffic.
2. Obtain independent sign-off on the resulting Release A promotion decision.
3. Keep Release B/v2 out of production until its SQL transaction, security, idempotency, concurrency, two-browser, and production-sized performance gates pass on staging.
