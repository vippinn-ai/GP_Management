...
For multi-hop checkout contention, the harness SHALL create a real three-session chain through two `hop_session` and two continuation `start_session` UI flows. Both browser contexts SHALL independently prepare the final checkout, capture one v2 command each, submit each distinct mutation exactly once, and perform no winner replay. Exactly one command SHALL commit; the loser SHALL return `session_not_billable` with a null mutation-status lookup. All three sessions and exactly three linked session-charge lines SHALL point to the one winning bill, the losing bill/event/audits SHALL be absent, authenticated actor attribution SHALL match the winning JWT, and both the `app_state` version and data hash SHALL remain unchanged across the financial race.
