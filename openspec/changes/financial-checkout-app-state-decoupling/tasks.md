...
- [x] Local unit and characterization tests pass (39 files / 450 tests on 2026-08-25).
...
- [ ] Run the dedicated receptionist/manager checkout-hop timing matrix. The reusable local harness now requires distinct ignored role credentials, authoritative active membership preflight before writes, both role directions across checkout-first/hop-first/concurrent orderings, exact loser codes, actor reconciliation, and single-send `Bill & Done` cleanup for a hopped winner; staging execution remains separately approval-gated.
