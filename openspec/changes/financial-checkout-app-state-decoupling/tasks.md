...
- [ ] Pass the reusable zero-retry checkout-versus-standalone-settlement race through `npm run test:e2e:staging:v2:checkout-settlement-race`; retain the single execution, exact winner/loser mutation evidence, one-payment reconciliation, actor attribution, guarded losing-session cleanup, and unchanged `app_state` version/hash.
