...
- [ ] Run the dedicated receptionist/manager checkout-hop timing matrix. Authorized run `role_matrix_20260825_1457` passed the distinct-role preflight and its first checkout-first product outcome, then stopped without retry on a harness assertion that incorrectly required a payment for a rounded zero-total bill; five cases did not run. Exact read-only reconciliation and final empty-floor preflight passed, both QA profiles were deactivated, and the conditional-payment assertion is corrected locally. A complete fresh run remains separately approval-gated.
