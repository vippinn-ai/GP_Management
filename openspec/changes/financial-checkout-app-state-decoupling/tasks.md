...
- [ ] Run the dedicated receptionist/manager checkout-hop timing matrix. Run `role_matrix_20260825_1457` proved the receptionist checkout-first product outcome before a zero-total payment assertion stopped the suite. Fresh run `role_matrix_20260825_162048` passed receptionist checkout-first and fully reconciled manager checkout-first before stopping on a delayed native-alert assertion after reload. Both runs were zero-retry, fully reconciled to empty floors, and used deactivated run-specific profiles. The two checkout-first role directions are demonstrated; four hop-first/concurrent cases remain separately approval-gated after the local alert-order correction.
- [ ] SQL transaction/security/idempotency/concurrency tests pass. Anonymous, wrong-organization, actor-spoof, malformed, inactive-user, rollback, authenticated financial/admin role boundaries, same/different-mutation, over-settlement, bill-number collision, exact- and over-capacity limited-stock contention, both admin inventory metadata orderings, both checkout/reject orderings, all three authoritative-admin checkout/hop orderings, and single- and multi-hop double-bill races pass; receptionist/manager checkout-hop timing outcomes and concurrency against remaining operational/financial-adjustment writers remain.
