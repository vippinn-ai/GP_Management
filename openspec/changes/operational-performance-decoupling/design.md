...
Invalidation actively rejects a still-pending realtime-ready barrier and unsubscribes its channel, so an unadopted preparation cannot hang until its ten-second timeout. Every subscription receives a unique generation token, so a delayed readiness or hydration completion from a disposed subscription cannot notify a remount even when the remount reuses the same callback identities. Installer preflight, install, generated preflight-bound postflight, standalone postflight, and rollback compare canonical function bodies with an executed CRLF/CR/LF normalization self-test and accept exactly one applicable `PERMISSIVE`, authenticated, tenant-helper policy for `operational_events`, including policies that become applicable through roles inherited by `authenticated`.
