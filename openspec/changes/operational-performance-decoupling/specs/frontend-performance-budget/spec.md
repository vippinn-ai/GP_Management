...
- **AND** baseline and candidate both expose the Activity UI under the same enabled feature flag, every completed Activity response succeeds with valid body evidence, and each candidate load has exactly one Dashboard Activity RPC across all statuses with a persisted finite positive request-start offset from safe interaction
