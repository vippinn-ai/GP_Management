...
2. If `current_setting('app.settings.api_url', true)` is absent, verify the visible SQL Editor URL is exactly `https://supabase.com/dashboard/project/tkbdyzxwwbhkpztgjjxh/sql/new`, run `operational-v2-staging-api-url-anchor.sql` once, and save its result. This staging-only bootstrap persists the exact approved project URL as a database-specific setting, refuses every non-exact existing URL or conflicting environment identity, and changes no application/domain rows. `ALTER DATABASE ... SET` is intentionally outside a transaction because PostgreSQL requires that; if a later statement is interrupted, rerun the exact file and its exact-match guard treats the already-persisted setting as an idempotent recovery. Never run it from a production tab and never edit its project reference.
