...
- Baseline and candidate evidence must use performance metric version 2. Readiness, request cutoff, dependency depth, and response end use one browser clock; Playwright observation delay is diagnostic only. Critical resources and responses are joined by hashed URL occurrence and fail closed on missing/duplicate/invalid timing. API payload is decoded bytes, cold shell is browser-reported transferred bytes including navigation, and JavaScript decoded/gzip sizes remain separate. All budget failures are emitted in the same completed evidence result.
