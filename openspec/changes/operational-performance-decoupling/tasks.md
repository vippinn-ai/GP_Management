...
- [x] Correct metric-v2 to use one in-page visible-dashboard comparison boundary for both builds while keeping candidate application-safe timing separate; fail closed across request/resource/response timing, status, transfer, body, and JSON evidence; test responsive Inventory layouts; preload its chunk post-safe; and remove the serial normalized organization lookup by resolving it beside the active profile without weakening subscription-first catch-up.
