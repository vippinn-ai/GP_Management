...
- [x] 10.1.5 Confirm the exact production feature-flag set before building; do not enable any flag in production that was not tested in staging
- [x] 10.1.6 Confirm production deploy command will use `npx wrangler deploy --name management`; do not use staging deploy commands for production
...
- [x] 10.3.1 Run `supabase/phase1-normalized-schema.sql` in production, then verify RLS is enabled on all normalized tenant tables
...
- [x] 10.3.3 Run `supabase/phase1-backfill-from-app-state.sql` in production during the write-pause window
...
- [x] 10.3.10 Confirm `authenticated` can execute browser-facing operational and financial RPCs
- [x] 10.3.11 Confirm helper functions such as `patch_app_state_array_by_id` and `resolve_operational_customer` are not directly executable by `anon` or `authenticated`
...
- [x] 10.4.2 Runtime cutover deploy: enable only the production flags that were already proven in staging
- [x] 10.4.3 Required bundled flags for operational RPC cutover: `VITE_BACKEND_RPC_OPERATIONAL_WRITES=true` and `VITE_BACKEND_NORMALIZED_LIVE_READS=true`
- [x] 10.4.4 Required bundled flag for financial RPC cutover: `VITE_BACKEND_RPC_FINANCIAL_WRITES=true`
- [x] 10.4.5 Enable `VITE_BACKEND_NORMALIZED_REALTIME` in production only after compact realtime implementation is completed and tested
...
- [x] 10.5.5 Pause and resume a session
...
- [x] 10.5.9 Issue a session bill and confirm `financial_checkout_committed` with `entity_type = session`
- [x] 10.5.10 Issue a customer-tab bill and confirm `financial_checkout_committed` with `entity_type = customer_tab`
- [ ] 10.5.11 Replace an issued bill and confirm `financial_checkout_committed` with `entity_type = bill`
- [x] 10.5.12 Void or refund a test issued bill and confirm `financial_adjustment_committed`
...
- [x] 10.6.3 Compare representative payload sizes against staging evidence; common RPC payloads should stay in the compact KB range, not MB range
...
- [x] 10.6.6 Keep production source on `app_state` compatibility; do not start Phase 6 retirement work during this rollout
