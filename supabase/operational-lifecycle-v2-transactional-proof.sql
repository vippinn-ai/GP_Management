...
select pg_temp.qa_assert((select count(*)=3 from public.operational_mutations m cross join qa_context c where m.organization_id=c.organization_id and m.mutation_id like c.run_id||'-mutation-%' and m.status='committed' and m.actor_id=c.actor_id), 'Committed mutation actor/count mismatch.');
...
select jsonb_build_object(
  ...
  'negative_cases',jsonb_build_array('same-id-mismatch','malformed','future-time','actor-spoof','late-audit-collision','inactive','anonymous','wrong-organization'),
