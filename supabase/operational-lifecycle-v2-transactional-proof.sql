...
select pg_temp.qa_assert((select count(*)=93 from public.audit_logs a cross join qa_context c where a.organization_id=c.organization_id and a.id like c.run_id||'-%audit-%' and a.id not like c.run_id||'-v1-audit-%' and a.action in ('session_hopped','session_rejected','customer_tab_rejected')), 'A successful v2 audit is missing or a failed v2 case left an audit row.');
